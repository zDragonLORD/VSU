...
Обработка спутникового снимка (выделение воды).Цель: Выделить водные объекты (сделать их белыми), остальное — черным.Диапазон воды: яркости от 40 до 80.Функция:$f(x) = 255$, если $x < 40$$f(x) = 255 - 255\frac{x - 40}{40}$, если $40 \le x < 80$$f(x) = 0$, если $x \ge 80$
